...
class UAV(object):
    ...
    def absolute_camera_coords(self):
        return self._coords + self._camera_coords @ self.rotation_matrix
...
save_path = '/Users/efeoz/Desktop/simulation_result'
...
uav_args = {'coords': (0., 0., 5e3),
            'rotation': (np.pi, 0.0, 0.0),
            "speed": 200,
            'camera_rotation': (0, 0, np.pi/2), # (roll(x), pitch(y), yaw(z)) in the order zyx wrt plane origin
            'camera_coords': (3.5, 0.0, 0.5) # (x,y,z) translation wrt plane origin
            }


camera = Camera(**cam_args)

uav = UAV(**uav_args, camera=camera)

sim = Simulator(uav_args=uav_args, cam_args=cam_args, samples_per_frame=15)

sim_real = Simulator(uav_args=uav_args, cam_args=cam_args, samples_per_frame=15)
...
ground_coords = np.zeros((len(controls)+1,3))
ground_rot = np.zeros((len(controls)+1,3))
real_coord=np.zeros((len(controls)+1,3))
real_rot = np.zeros((len(controls)+1,3))
error_coords = np.zeros((len(controls)+1,3))
error_rot = np.zeros((len(controls)+1,3))
...
for k in range(len(controls)):
        ...
        control_input = {'droll_dt': ctrl[0],
                         ...
                         'dt': 1e-20,
                         'radians': False}
        sim._uav.applyControl(**control_input)
        sim_real._uav.applyControl(**control_input)

        errs_angles, errs_coords, failures, setting_text, fname = sim.estimatePnPErrorDistribution(
            resolution_scaler=1.,
            gps_error_level='medium',
            calibration_error_level='medium',
            elevation_range='large',
            method='dlt')
        estimated_coords = sim._uav.coords + errs_coords
        estimated_rot = sim._uav.euler_rotation_zyx + errs_angles
        sim._uav.setPose(rotation=estimated_rot, coords=sim._uav.coords)

        control_input2 = {'droll_dt': 0.,
                          'dpitch_dt': 0.,
                          'dyaw_dt': 0.,
                          'duration': duration,
                          'dt': dt,
                          'radians': False}
        sim._uav.applyControl(**control_input2)
        sim_real._uav.applyControl(**control_input2)


        real_coord[k+1,:] = sim._uav.coords

        real_rot[k+1 ,:] = (sim._uav.euler_rotation_zyx * 180 / np.pi) % 360


        ground_coords[k+1,:] = sim_real._uav.coords

        ground_rot[k+1,:] = (sim_real._uav.euler_rotation_zyx * 180 / np.pi) % 360


        error_coords[k+1,:] = sim._uav.coords - sim_real._uav.coords

        error_rot[k+1 ,:] = sim._uav.euler_rotation_zyx - sim_real._uav.euler_rotation_zyx
...
df= pd.concat([pd.DataFrame(real_coord,columns = ['real_x', 'real_y','real_z']),pd.DataFrame(ground_coords,columns = ['ground_x', 'ground_y','ground_z']),
               pd.DataFrame(real_rot,columns = ['real_roll', 'real_pitch','real_yaw']),pd.DataFrame(ground_rot,columns = ['ground_roll', 'ground_pitch','ground_yaw'])], axis=1, join='inner')
...
plt.figure(1)
plt.scatter(real_coord[:,0],real_coord[:,1])
plt.scatter(ground_coords[:,0],ground_coords[:,1])

plt.title("Real and Ideal Coordinates for High Error", fontsize=10)
plt.xlabel("Latitude Coordinates(m)", fontsize=10)
plt.ylabel("Longitude Coordinates(m)", fontsize=10)
plt.legend(["Real Coordinates","Ideal Coordinates"])
...
plt.figure(1)
plt.scatter(sim_real._uav._camera.field_of_view[:,0],sim_real._uav._camera.field_of_view[:,1],c="b")
#plt.plot(sim._uav._camera.field_of_view[(0,3), 0], sim._uav._camera.field_of_view[(0,3), 1],c="b")
plt.scatter(final_estimated_coords[:,0],final_estimated_coords[:,1])
plt.scatter(sim_real._uav.coords[0],sim_real._uav.coords[1])
plt.grid()
plt.xlabel("x (m)")
plt.ylabel("y (m)")
plt.legend(["FOV","Estimated coords","True coords"])

plt.figure(2)
plt.plot(real_coord_nhop[:,0],real_coord_nhop[:,1], linewidth=2,marker='o')


plt.title("The way the plane follows ideally", fontsize=10)
plt.xlabel("Latitude Coordinates(m)", fontsize=10)
plt.ylabel("Longitude Coordinates(m)", fontsize=10)

plt.figure(3)

plt.title("The Mean Value of the Coordinate Errors After Each Hop", fontsize=10)
plt.xlabel("Hop Number", fontsize=10)
plt.ylabel("The Mean Value of the Coordinate Errors(m)", fontsize=10)
plt.plot(error_coords_mean[:,0], linewidth=2,marker='o')
plt.plot(error_coords_mean[:,1], linewidth=2,marker='o')
plt.plot(error_coords_mean[:,2], linewidth=2,marker='o')

plt.grid(which='both', alpha=0.2)
plt.legend(["Latitude Coordinates", "Longitude Coordinates","Altitude Coordinates"], loc ="lower right",prop={'size': 6})

plt.figure(4)

plt.title("The Standart Deviation Value of the Coordinate Errors After Each Hop", fontsize=10)
plt.xlabel("Hop Number", fontsize=10)
plt.ylabel("The Standart Deviation Value of the Coordinate Errors(m)", fontsize=10)
plt.plot(error_coords_std[:,0], linewidth=2,marker='o')
plt.plot(error_coords_std[:,1], linewidth=2,marker='o')
plt.plot(error_coords_std[:,2], linewidth=2,marker='o')
